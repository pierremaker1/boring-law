...
| **Flag** | Avertissement attaché à une question (`questions.flag`) : le cours et le droit en vigueur divergent. Laquelle des deux versions la question retient dépend du format de l'examen (le cours en CEDH, le droit positif en droit fiscal) et le texte du flag le dit. Affiché en révision (« ⚠️ Ton cours et le droit en vigueur divergent ») et filtrable via *À surveiller*. | `isFlagged()` (`ReviewList.tsx`) |
